...
#this function returns a BED-formatted string for the feature passed as region with blocks defining the exons as per the BED file format 
    def blockBedFormat(self, region="exons"):
        ...
        with open("parameters.txt",'r') as fh:
            value=[ keys for keys in fh.readlines() if ("transcript_delimiter" in keys) ]
            delimiter=str(value[0].split("=")[1]).rstrip('\n')
	
            if (region == "5utr"):
                chromStart = self.utr5[0][1] #- 3 # start of feature is start of first block ## -3 if you want to include the start codon
                chromEnd = self.utr5[-1][2] # end of feature is end of last block 
                regionName = self.name + delimiter +"5utr"

                ##CHECK if I should fetch flanking sequences for the 5 UTR
                if((chromEnd-chromStart)/3==1):
                    #chromStart = chromStart - 2000 #We want 2kb of flanking region ##OLD CODE, using bedtools now
                    regionName = self.name + delimiter + "5utr_FLANK"
            ...
            #print "blockCount %d blockSizes %s blockStarts %s" % (blockCount, blockSizes,  blockStarts)
                ...
                regionName = self.name + delimiter + "5utr_start"
                ...
                regionName = self.name + delimiter + "cds"
                ...
                regionName = self.name + delimiter + "3utr"

                blockCount = len(self.utr3)
                blockSizes = ''.join(["%d," % ((chunk[2]-chunk[1])) for chunk in self.utr3])
                blockStarts = ''.join(["%d," % ((chunk[1]-chromStart)) for chunk in self.utr3]) ## +3 to fix block start value in bed file

            #if self.name == "ENSDART00000179357":
            #    print(chromStart,chromEnd, chromEnd-chromStart, (chromEnd-chromStart)/3, (chromEnd-chromStart)%3)
            #    print(chunk[1], chunk[2])
            #    print(chunk)
            #    print(self.utr3)

            ##CHECK if I should fetch flanking sequences for the 3 UTR
                if((chromEnd-chromStart)/3==1):
                    #chromEnd = chromEnd + 2000 #We want 2kb of flanking region ##OLD CODE, using bedtools now
                    regionName = self.name + delimiter + "3utr_FLANK"
                ...
                regionName = self.name + delimiter + "exon"
                ...
                regionName = self.name + delimiter + "intron"
