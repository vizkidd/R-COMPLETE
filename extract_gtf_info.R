...
parallel_extract_info <- function(slice_file, strandedness="+") {
  ...
  gtf_split <- split(gtf, as.factor(g_name))
  ...
  return(lapply(gtf_split, function(gtf_x){
          gtf_gr <- GRanges(
          ...
          if (strandedness=="+" || strandedness=="-") {
            ...
            if(is.null(gtf_gr) || length(gtf_gr)==0 || is.na(gtf_gr)){ ##Probably the mrna is not in the + strand so we can safely discard it
              print(paste("No",strandedness,"strand info (or) region of gtf missing for the gene : ", g_name))
              return()
            }
          }
...
get_bed <- function(bed_df, BED_PATH_PREFIX, TRANSCRIPT_REGIONS) {
  if(nrow(bed_df) > 0 || !is.null(bed_df)){
    ...
    bed_list_names <- tolower(names(bed_split))
    #bed_list_names[which(bed_list_names=="three_prime_utr")] <- "3utr"
    #bed_list_names[which(bed_list_names=="five_prime_utr")] <- "5utr"
    #names(bed_split) <- bed_list_names
...
get_stats_parallel <- function(slice_file_path, STRAND, n_cores, org_name, output_file, TRANSCRIPT_ID_DELIM, BED_PATH_PREFIX, TRANSCRIPT_REGIONS) {
  ...
  utr_len_list <- mcmapply(FUN=parallel_extract_info, list.files(path=slice_file_path, pattern = "*.gtf_slice",full.names = T), MoreArgs=list(strandedness=STRAND), USE.NAMES = F, mc.cores = n_cores-1, SIMPLIFY = F, mc.cleanup=T) # MoreArgs=list( output_path=output_path, org_name=org_name) MoreArgs=list(strandedness=STRAND, transcript_delim=TRANSCRIPT_ID_DELIM) mc.preschedule = T
  ...
  if(nrow(utr_len_df) > 0 || length(utr_len_df) > 0 || !is.null(utr_len_df)){
    ...
    bed_df <- bind_rows(lapply(utr_len_list, function(x){
        #tmp_lens <- x[["utr_len"]]
        tmp_bed <- x[[2]] #x[["bed_data"]] #return(x[[2]])
...
if(!file.exists(output_file) || file.info(output_file)$size<4 || !file.exists(paste(BED_PATH_PREFIX,"bed",sep=".")) || file.info(paste(BED_PATH_PREFIX,"bed",sep="."))$size<4){
  ...
  #if (file.exists(paste(BED_PATH_PREFIX,"bed",sep=".")) && file.info(paste(BED_PATH_PREFIX,"bed",sep="."))$size>0) {
