...
mart_connect <- function(MART_FUN=NULL,args=c(),verbose=T){
  if (!is.null(MART_FUN)) {
    ...
    while (time_out < 600) { #max time out is 10mins, code will fibonacci to it stepping up 5 seconds
      ...
      if(!any(grepl(pattern = "error|exception",ignore.case = T,x = class(catch_value))) && !is.null(catch_value)){
        #print(class(catch_value))
        #print("Done!")
        return(catch_value)
      }
    }
    stop("Ensembl is unresponsive, please check connection\n")
  }else{
    return(0)
  }
}
...
check_files <-function(fasta_path,org,genes){
  ...
  #   return(FALSE)
  # }
  if(dir.exists(fasta_path) && length(dir(fasta_path)) > 0 && length(grep(list.files(path=fasta_path), pattern="tmp", invert=T, value=TRUE)) > 0){
    missing_genes <- c()
    available_genes <- c()
...
get_FASTA_mart <- function(org,gtf_stats, fasta_path){
  ...
  missing_all_flank_info <- unique( invalid_stats$transcript_id[apply(X = (invalid_stats[,c(grep("five",colnames(invalid_stats),ignore.case = T,value = F),grep("three",colnames(invalid_stats),ignore.case = T,value = F))] ==0) , MARGIN = 1, FUN = all)] ) #intersect(na.omit(invalid_stats$transcript_id[invalid_stats[,grep("five",colnames(invalid_stats),ignore.case = T,value = F)] == 0]), na.omit(invalid_stats$transcript_id[invalid_stats[,grep("three",colnames(invalid_stats),ignore.case = T,value = F)] == 0]) )
  
  flank_stats <- invalid_stats #invalid_stats[is.na(match(invalid_stats$transcript_id,missing_all_flank_info)),]  
  if (nrow(flank_stats) > 0) {
    if (length(missing_all_flank_info) > 0) { ##All these transcripts have neither UTR Lengths nor Flank lengths info (possibly because the gene does not have isoforms or info is missing), so I set the utr flanks to transcript lengths and correct for variance
      arbitrary_flanks <- invalid_stats[!is.na(match(invalid_stats$transcript_id,missing_all_flank_info)),]  
      arbitrary_flanks$three_flank <- arbitrary_flanks$five_flank <- arbitrary_flanks$transcript_length
...
fetch_genome_user <- function(data){
  ...
  if(CLEAN_EXTRACT || !try(check_files(org_fasta_path,org,genes)) && !is.logical(gtf_path) && !is.logical(genome_path)){
...
print("Checking and downloading FASTA...")
