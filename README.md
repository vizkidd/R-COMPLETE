...
directory, + Download [OrthoDB(ODB) files](#odb) (optional) and
[Tools](#tools) + Check [config files](#files) + Provide paths and
options in the parameters file + **NOTE : Default parameters file
([parameters.txt](inst/pkg_data/parameters.txt)) is at**
...
chosen, + To expose as many options as possible to the end-user + The
pipeline uses BASH to BLAST and handle files (significantly faster than
R) and the parameter file is shared between R and BASH.\
+ Parameters tagged as output in comment column are outputs from
COMPLETE
