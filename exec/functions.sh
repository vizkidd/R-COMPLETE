...
function get_fasta() {
	...
	if [ ! -s $FASTA_PATH/$s_name.$reg ]; then #$FASTA_PATH/$file_out.cds
	  ...
	  if [[ -s "$base_bed"_"$reg.bed" ]]; then
	  	grep -i -w $gene_full $OUT_PATH/$org_name/gtf_stats.csv | awk -F',' '{print $3}' | grep -w -f - "$base_bed"_"$reg.bed" > $TEMP_PATH/"$s_name"_"$reg.bed"
	  fi
	  ...
	  if [[ -s $TEMP_PATH/"$s_name"_"$reg.bed" ]]; then 
	    ...
	    if [[ "$reg" == "3utr" ]]; then
	      local flank_len=$(grep -w -i "$gene_full" $OUT_PATH/$org_name/gtf_stats.csv | awk -F',' '{ if ($9 > 3) sum += $9; n++ } END { if (n > 0) print sum / n; }') ##3' UTR length must be > 3 (because of existence of stop codons)
	      if [[ $flank_len == "" ]]; then
	        return 2
	      fi
	      bedtools flank -s -l $flank_len -r 0 -i $TEMP_PATH/"$s_name"_cds.bed -g $genome_fa.fai > $TEMP_PATH/"$s_name"_"$reg"_FETCH.bed 
	    elif [[ "$reg" == "5utr" ]]; then
	      local flank_len=$(grep -w -i "$gene_full" $OUT_PATH/$org_name/gtf_stats.csv | awk -F',' '{ if ($8 > 0) sum += $8; n++ } END { if (n > 0) print sum / n; }') ##5' UTR length must be > 0
	      if [[ $flank_len == "" ]]; then
	        return 2
	      fi
...
function accumulate_clusters(){
	...
	# $3 - Output file ($OUT_PATH/danio_rerio/ALL_CLUSTERS)
...
function group_FASTA_clusters(){
	...
	grep -r -h ">" $f_org  | awk -F"$seqID_delimiter" '{print $NF}' | awk '{split($0,a,","); for(key in a) print a[key];}' | sort -u > $OUT_PATH/$f_org_name/ORG_CLUSTERS
	parallel --max-procs $n_threads " printf '%s\t%s\n' {1} {2}" :::: <(grep -H -f $OUT_PATH/$f_org_name/ORG_CLUSTERS -r $FASTA_PATH/$f_org_name/ | awk -F'[:>]' -v s_delim="$seqID_delimiter" '{split($2,a,s_delim); n=split($1,b,"."); print $1"\t"$2"\t"a[5]"\t"b[n]'}) | parallel  --max-procs 1 --colsep '\t' --recend '\n'  "if [[ -s {1} && ! -z {2} && ! -z {1} && ! -z {3} ]] ; then samtools faidx {1}  {2} >> $GROUPS_PATH/{3}.{4} ; fi" 
done
}
	...
	if [ ${#script_args[@]} -eq 4 ]
	  then
	    if ! gunzip -t "$ORTHODB_PATH_PREFIX"_OGgenes_fixed_user.tab.gz &> /dev/null ; then
			time zgrep -f <(zcat -f $gene_list | sort | uniq | grep -v -w -i "gene") "$ORTHODB_PATH_PREFIX"_OGgenes_fixed.tab.gz | gzip -c > "$ORTHODB_PATH_PREFIX"_OGgenes_fixed_user.tab.gz
		else
			>&1 color_FG $Green "File exists!"	
		fi
...
function extract_genomic_regions(){
	...
	if [[ $CLEAN_EXTRACT ==  "TRUE" ]] ; then
	  ...
	  rm -rf $OUT_PATH/$f_org_name
	  rm -rf $TEMP_PATH/$f_org_name
	  rm -f $GENOME_FILE.fai
	  rm -f $OUT_PATH/$f_org_name/*
	  rm -f $bed_prefix/*
	fi
	...
	local gene_list=($(cat $GENE_LIST | sort | uniq | grep -v -w -i "gene"))
	...
	if [[ ! -s $OUT_PATH/$f_org_name/odb.list || ! -s $OUT_PATH/$f_org_name/final.list || ! -s $OUT_PATH/$f_org_name/odb.final_map ]] ; then
	  time check_OrthoDB $f_org_name $GENE_LIST $OUT_PATH/$f_org_name/odb.list $OUT_PATH/$f_org_name/odb.final_map $param_file
	fi

	if [[ $(awk 'END{print NR;}' "$OUT_PATH/$f_org_name/odb.list" | awk '{print $1}') !=  0 ]] ; then
	    local odb_gene_list=($(cat "$OUT_PATH/$f_org_name/odb.list" | grep -v -w -i "gene"))
	    local existing_list=($(cat $OUT_PATH/$f_org_name/1.list | sort | uniq)) 
	  ...
	  cat $OUT_PATH/$f_org_name/1.list $OUT_PATH/$f_org_name/2.list $OUT_PATH/$f_org_name/odb.list | awk 'NF' > $OUT_PATH/$f_org_name/full.list
	...
	time Rscript --vanilla --verbose $(echo $(dirname $0))/extract_gtf_info.R $TEMP_PATH/$f_org_name/ $f_org_name $OUT_PATH/$f_org_name/gtf_stats.csv $param_file #1> $TEMP_PATH/$f_org_name/get_GTF_info.o 2> $TEMP_PATH/$f_org_name/get_GTF_info.e
	r_exit_code="$?"
	sed 1d $OUT_PATH/$f_org_name/gtf_stats.csv | awk -F',' '{print $1"\n"}' | sort | uniq | awk 'NF' > $OUT_PATH/$f_org_name/final.list
	...
	if [[ ! -s $OUT_PATH/$f_org_name/gtf_stats.csv || ! -s $OUT_PATH/$f_org_name/final.list || $r_exit_code != 0 ]] ; then
	  ...
	  >&2 color_FG_Bold $Red "3. (Possible Fix) : Remove $TEMP_PATH/$f_org_name/ & $OUT_PATH/$f_org_name/gtf_stats.csv and re-run the pipeline"
	  exit 1
	fi
	...
	local gene_list=($(cat $OUT_PATH/$f_org_name/final.list | awk 'NF' ))
	...
	#############################################################################################################
	...
	#if [[ $LABEL_SEQS ==  "TRUE" && -s $OUT_PATH/$f_org_name/odb.final_map ]] ; then
	  ...
	  time parallel --max-procs $n_threads "printf -- %s,%s\\\n {1} {2}" ::: ${tmp_names[@]} ::: ${TRANSCRIPT_REGIONS[@]} | parallel --colsep "," --max-procs $n_threads "label_sequenceIDs $f_org_name {1} $FASTA_PATH/$f_org_name/{2}.{3} $FASTA_PATH/$f_org_name/{2}.{3}.tmp $param_file $OUT_PATH/$f_org_name/odb.final_map" 
	#fi
	...
	sed 1d $OUT_PATH/$f_org_name/gtf_stats.csv | awk -F',' '{print $1}' | sort | uniq >  $OUT_PATH/$f_org_name/AVAILABLE_GENES 
	grep -v -i -f $OUT_PATH/$f_org_name/AVAILABLE_GENES $GENE_LIST | sort | uniq > $OUT_PATH/$f_org_name/MISSING_GENES
...
function check_OrthoDB(){
	...
	if ! gunzip -t "$ORTHODB_PATH_PREFIX"_OGgenes_fixed_user.tab.gz ; then
	  ...
	  >&2 color_FG_Bold $Red "2. Error in ODB files! Rerun merge_OG2genes_OrthoDB.sh $ORTHODB_PATH_PREFIX $CLEAN_EXTRACT $n_threads ${script_args[1]}"
	  exit -1
	else
	...
	local s_names=($(awk  -v s_var='_' '{ gsub(/[[:punct:]]/,s_var);}1' <(echo "${gene_list[@]}")))
	local genes_strip=($(awk -F'_' '{print $(echo '$1')}' <(echo "${s_names[@]}")))
	...
	zgrep "$org_ID" $TEMP_PATH/$f_org_name/odb.clusters.gz | awk '{split($3,a,","); for(key in a){print $1"\t"a[key]}}' | grep -i $MODE -f <(cat ${script_args[1]} $out_gene_list | grep -v -w -i "gene") | sort -u | awk '{if($1 in a){a[$1]=a[$1]","$2}else{a[$1]=$2;} } END{for(key in a){print key"\t"a[key]}}' > $out_gene_clusters #> $OUT_PATH/$f_org_name/odb.final_map

	if [[ -s $out_gene_clusters ]]; then #$OUT_PATH/$f_org_name/odb.final_map
