...
full_cmd=$@
sleep_time=100 # seconds; don't make this too short! don't want to tax system with excessive qstat calls
me=`whoami`
...
if [[ $(builtin type -P "qsub" | wc -l) == 1 ]]; then 
    CLUSTER="SGE"
    CLUSTER_QUEUE_CMD="qsub -V -o $TMP_DIR/$1.o -e $TMP_DIR/$1.e -N $1 $tmp_file" #time ${full_cmd[@]/$1}"
    CLUSTER_JOBS_CMD="qstat | grep $me"
elif [[ $(builtin type -P "sbatch" | wc -l) == 1 ]]; then 
    CLUSTER="SLURM"
    CLUSTER_QUEUE_CMD="sbatch --job-name $1 $tmp_file" #time ${full_cmd[@]:1}"
...
do
    sleep $sleep_time
    echo "Job $id($1)($tmp_file) running..."
	status=`myqstat | grep $id`
    #status=`$CLUSTER_JOBS_CMD | grep $id`
    #echo $status
done
echo "----Job $id($1)($tmp_file) complete---- "